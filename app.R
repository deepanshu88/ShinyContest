      ...
      observe({
        ...
        output$suicide <- renderUI( {
          ...
          if(input$country=='Overall') {
            y <- scales::number(mean(overall_tibble$s1), accuracy = 0.01)
          }
        ...
        output$suicide4 <- renderUI( {
          ...
          if(input$country=='Overall') {
            y <- overall_tibble %>% filter(s1 == max(overall_tibble$s1)) %>% pull(year)
          }
        ...
        output$line <- renderHighchart({ 
            ...
            highchart() %>% 
            hc_add_series(overall_tibble, hcaes(x = year, 
                                                y = s1,
                                                color = s1), type = "line") %>%
